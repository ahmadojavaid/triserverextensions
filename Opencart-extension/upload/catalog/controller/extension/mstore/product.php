...
class ControllerExtensionMstoreProduct extends Controller {
	...
	public function index() {
		...
		$results = $this->model_extension_mstore_product->getProducts($filter_data);
		$data = array();
		...
		foreach ($results as $result) {
			...
			foreach ($images as $image) {
				if ($image['image']) {
					$thumbnails[]= $this->model_tool_image->resize($image['image'], 600, 600);
				} else {
					$thumbnails[] = $this->model_tool_image->resize('placeholder.png', 600, 600);
				}
			}
